...
/* loaded from: oauth2-module-authentication-3.5.7.0.jar:cn/herodotus/engine/oauth2/authentication/provider/AbstractAuthenticationConverter.class */
public abstract class AbstractAuthenticationConverter implements AuthenticationConverter {
    ...
    protected String[] decrypt(HttpServletRequest request, String sessionId, List<String> parameters) {
        if (SessionUtils.isCryptoEnabled(request, sessionId) && CollectionUtils.isNotEmpty(parameters)) {
            List<String> result = parameters.stream().map(item -> {
                return decrypt(request, sessionId, item);
            }).toList();
    ...
    protected String decrypt(HttpServletRequest request, String sessionId, String parameter) throws OAuth2AuthenticationException {
        if (SessionUtils.isCryptoEnabled(request, sessionId) && StringUtils.isNotBlank(parameter)) {
            try {
                ...
                OAuth2EndpointUtils.throwError(OAuth2ErrorKeys.SESSION_EXPIRED, e.getMessage(), OAuth2EndpointUtils.ACCESS_TOKEN_REQUEST_ERROR_URI);
            }
        }
        return parameter;
    }
    ...
    protected Map<String, Object> getAdditionalParameters(HttpServletRequest request, MultiValueMap<String, String> parameters) {
        ...
        parameters.forEach((key, value) -> {
            if (!key.equals("grant_type") && !key.equals("scope")) {
                additionalParameters.put(key, value.size() == 1 ? decrypt(request, sessionId, (String) value.get(0)) : decrypt(request, sessionId, (List<String>) value));
            }
        });
    ...
    protected Set<String> getRequestedScopes(String scope) {
        ...
        if (org.springframework.util.StringUtils.hasText(scope)) {
            requestedScopes = new HashSet<>(Arrays.asList(org.springframework.util.StringUtils.delimitedListToStringArray(scope, SymbolConstants.SPACE)));
        }
